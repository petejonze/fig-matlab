...
%                   child handles
%
%           add \vphantom{X} to all legend entries, to ensure correct
%           vertical spacing
%
%         % nudge legend text down
%         for i = 1:4
%             set(hChildren.hLegText(i),'Position', get(hChildren.hLegText(i),'Position')+[0 -.04 0]);
%         end
%
...
% Uncomment to add latex formatting by default
    ...
    try
        ...
        if ~isempty(hLeg) && ~isempty(legTitle)
            hTitle = legendTitle(hLeg,legTitle);
        end
        ...
        if ~isempty(hChildren.hLegPatch) % if there are any patches..
            ...
            if ~isempty(regexpi(loc,'west')) % compress holding left point constant
                tmp = get(hChildren.hLegText(1),'position');
                txtBuf = tmp(1) - XData(3); % get current gap between text and line
        ...
        if (~isempty(hChildren.hLegLines) || ~isempty(hChildren.hLegPatch)) && ~isempty(regexpi(loc,'west'))
            ...
            switch numel(XData)
                case 2
                    p = cellfun(@(x)([XData(2) + txtBuf x(2:end)]), p, 'UniformOutput',false);
                case 3
                    p = cellfun(@(x)([XData(3) + txtBuf x(2:end)]), p, 'UniformOutput',false);
                case 4
                    p = cellfun(@(x)([XData(3) + txtBuf x(2:end)]), p, 'UniformOutput',false);
            end
