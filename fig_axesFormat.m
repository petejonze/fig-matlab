function [hXTickLbl, hYTickLbl, c_axes, hxAxisTitle, hyAxisTitle, axesHandle] = fig_axesFormat(axesHandle, xTick,xTickLbl, yTick,yTickLbl, xAxisTitle,yAxisTitle, xlims,ylims, fontSize, formatData, xMinorTick,yMinorTick,mrTickLgth, xRotation,yRotation, vGridColr,hGridColr, tickLength_cm)
...
%
% I think currently minor tick mark lenght is (incorrectly) specified in
% data units rather than 0-1 normalized units (!)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if nargin < 19 || isempty(tickLength_cm)
            tickLength_cm = 0.15;
        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % X TICKS
        if isnumeric(axesHandle) && isnan(axesHandle)
        ...
        if nargin < 14 || isempty(mrTickLgth)
            mrTickLgth = [2 .55]; % proportion of major 2D tick length
        end
        if length(mrTickLgth)==1
            mrTickLgth = [mrTickLgth mrTickLgth];
        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if length(axesHandle) > 1
            for i = 1:length(axesHandle)
                axes(axesHandle(i));
                fig_axesFormat(axesHandle(i), xTick{i},xTickLbl{i}, yTick{i},yTickLbl{i}, xAxisTitle,yAxisTitle, xlims,ylims, fontSize, formatData, xMinorTick,yMinorTick,mrTickLgth, xRotation,yRotation, vGridColr,hGridColr, tickLength_cm);
            end
            return;            
        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if isnumeric(axesHandle) && isnan(axesHandle)
            ...
            for i = 1:length(handles)
                ...
                [hXTickLbl{i}, hYTickLbl{i}, c_axes(i)] = fig_axesFormat(gca, xTick,xTickLbl, yTick,yTickLbl, xAxisTitle,yAxisTitle, xlims,ylims, fontSize, formatData, xMinorTick,yMinorTick,mrTickLgth, xRotation,yRotation, vGridColr,hGridColr, tickLength_cm);
            end
            return;
        end
...
%             end
            ...
            if ~isempty(hXTickLbl)
                if length(hXTickLbl)==1
                    tmp = hXTickLbl.String;
                else
                    tmp = {hXTickLbl.String};
                end
                tag = sprintf('%s£x=%1.19f$',tag,tmp{:});
            end
            if ~isempty(hYTickLbl)
                    if length(hYTickLbl)==1
                        tmp = hYTickLbl.String;
                    else
                        tmp = {hYTickLbl.String};
                    end
                    tag = sprintf('%s£y=%1.19f$',tag,tmp{:});
            end
            set(fh,'Tag',tag);
...
%                 ,'TickDir'      , 'in'          ...            
            ...
            if strcmpi(get(axesHandle,'XScale'),'log') || strcmpi(get(axesHandle,'YScale'),'log')
                ...
                if ~isempty(children)
                    % delete text also as don't want duplciates of every
                    % textfield.. [break axes be damned!]
%                     z = get(children,'type');
...
%             set(axesHandle, 'visible', 'off');
            ...
            if ~isempty(xMinorTick)
                mrTickLgth_abs = majorTickLength * (ylims(2)-ylims(1)) * mrTickLgth(1);
                hold on
                for x = xMinorTick
            ...
            if ~isempty(yMinorTick)
                mrTickLgth_abs = majorTickLength * (xlims(2)-xlims(1)) * mrTickLgth(2);
                hold on
                for y = yMinorTick
