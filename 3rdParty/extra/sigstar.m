function varargout=sigstar(groups,stats,nosort,showExactPVal)
    ...
    if isempty(stats) || isempty(stats)
        stats=repmat(0.05,1,length(groups));
    end
    if nargin<3 || isempty(nosort)
        nosort=0;
    end
    if nargin<4 || isempty(showExactPVal)
        showExactPVal = false;
    end
    ...
    for ii=1:length(groups)
        ...
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),showExactPVal);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,showExactPVal)
